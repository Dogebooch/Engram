...
ledger of manual skips/flags in `ingest_queue.json` next to this script. A run that
...
def fetch_videos(conn: sqlite3.Connection, source: str | None, course: str | None) -> list[sqlite3.Row]:
    clauses: list[str] = []
    params: list[object] = []
    if source:
...
def cmd_next(args: argparse.Namespace) -> int:
    ...
    pending = [r for r in rows if int(r["id"]) not in skips and not is_done(root, r["path"])]
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    done = pending = skipped = flagged = 0
    for r in rows:
        src = str(r["source"])
        bucket = per_source.setdefault(src, {"total": 0, "done": 0, "pending": 0, "skipped": 0})
        bucket["total"] += 1
        vid = int(r["id"])
        if vid in flags:
            flagged += 1
...
def _set_ledger_entry(args: argparse.Namespace, bucket: str, value: str | None) -> int:
    ...
    save_ledger(ledger)
    print(json.dumps({"id": args.id, "title": row["title"], "action": action, "value": value}, ensure_ascii=False))
    return 0
...
def cmd_reset(args: argparse.Namespace) -> int:
    ...
    print(json.dumps({"id": args.id, "title": row["title"], "slug": slug, "removed": removed, "requeued": True}, ensure_ascii=False))
    return 0


def main() -> int:
    parser = argparse.ArgumentParser(description="Resumable Engram ingest queue (derived from MVS index + built zips).")
    ...
    p_status = sub.add_parser("status", help="Progress counts with a per-source breakdown.")
    ...
    p_flag = sub.add_parser("flag", help="Mark a video for later review (still counts as done).")
    ...
    p_reset = sub.add_parser("reset", help="Delete a video's built zip so it re-queues.")
    p_reset.add_argument("id", type=int)
    p_reset.add_argument("--purge", action="store_true", help="Remove the whole run dir, not just the zip.")
