...
function authorPrompt(video, critique) {
  ...
  return [
    ...
    `- run dir: ${runDir}`,
    ``,
    `Read .claude/skills/ingest-video/SKILL.md first and follow its facts-only steps 1–6 for THIS video. Do not deviate from these points:`,
    ``,
    `1. EXTRACT FRAMES (no transcript):`,
    `   ${PY} ${TOOLS}\\ingest_video.py --video "${video.path}" --out-root "${outRoot}" --skip-transcript`,
    `   Then Read ${runDir}\\keyframes.json. If it has <= 2 keyframes (static scene — common on Picmonic), re-extract a spread:`,
    `   ${PY} ${TOOLS}\\ingest_video.py --video "${video.path}" --out-root "${outRoot}" --skip-transcript --sample-seconds 2 --min-gap-seconds 3 --diff-threshold 0 --max-keyframes 18`,
    ``,
    ...
    ``,
    `4. Read candidate keyframes in ${runDir}\\frames\\ and pick the backdrop; classify the scene:`,
    `   - tableau (most Sketchy/Pixorize): one late frame holds every symbol.`,
    `   - picmonic: illustration shows mid-playback and the video ENDS on a text review page, so the LAST frames are the wrong pick — choose the mid/late frame showing the full illustration.`,
    `   - pan: no single frame holds everything — pick the most complete frame, author ONLY symbols visible in it, and list the narrated symbols that live on other pans in caveats.`,
    `   Record the chosen keyframe index for --backdrop-index.`,
    `   If EVERY candidate frame is black/blank/corrupt (a codec failure seen on some .mov files), set backdropUsable=false and note it in caveats — still author from the transcript and build, but the backdrop will need manual replacement.`,
    ``,
